...
@export var control_pull_strength: float = 18.0
@export var control_velocity_match: float = 1.2
@export var control_max_force: float = 24.0
@export var control_correction_speed: float = 5.0
@export var control_max_extra_speed: float = 1.8
@export var control_min_front_distance: float = 0.55
@export var control_lateral_snap: float = 8.0
...
func _control_balls(delta: float) -> void:
	...
	for body in _kick_area.get_overlapping_bodies():
		if body is RigidBody3D:
			...
			var relative := ball_pos - player_pos
			var forward_distance := relative.dot(facing)
			var lateral_distance := relative.dot(right)
			var snap_alpha := clampf(control_lateral_snap * delta, 0.0, 0.35)
			var keep_in_front := maxf(control_min_front_distance - forward_distance, 0.0)
			var lane_correction := (-right * lateral_distance + facing * keep_in_front) * snap_alpha
			ball.global_position += Vector3(lane_correction.x, 0.0, lane_correction.z)
			ball_pos += lane_correction
			var to_target := target - ball_pos
			var follow_velocity := Vector3(velocity.x, 0.0, velocity.z)
			var corrective_velocity := to_target * control_correction_speed
			var desired_velocity := follow_velocity + corrective_velocity
			var ball_velocity := ball.linear_velocity
			ball_velocity.y = 0.0
			var speed_cap := maxf(follow_velocity.length() + control_max_extra_speed, 3.2)
			if desired_velocity.length() > speed_cap:
				desired_velocity = desired_velocity.normalized() * speed_cap
			if ball_velocity.length() > speed_cap:
				var capped_velocity := ball_velocity.normalized() * speed_cap
				ball.linear_velocity = Vector3(capped_velocity.x, ball.linear_velocity.y, capped_velocity.z)
				ball_velocity = capped_velocity
